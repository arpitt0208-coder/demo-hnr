...
export function BlogPostCard({ post, index = 0 }: BlogPostCardProps) {
  ...
  return (
    <motion.article
      ...
      className="flex h-full flex-col overflow-hidden rounded-[16px] bg-white shadow-[0_4px_24px_rgba(15,23,42,0.08)]"
    >
      <div className="relative aspect-[4/3] w-full overflow-hidden">
        ...
        {post.featured ? (
          <span className="absolute left-3 top-3 inline-flex items-center gap-1.5 rounded-full bg-primary-yellow px-2.5 py-1 text-[10px] font-bold text-dark-navy shadow-sm sm:left-4 sm:top-4 sm:px-3 sm:py-1.5 sm:text-[11px]">
            <Star className="size-3 fill-dark-navy" strokeWidth={0} aria-hidden="true" />
            Featured
          </span>
        ) : null}
      </div>
      ...
      <div className="flex flex-1 flex-col p-5 sm:p-6">
        ...
        <h3 className="mt-3 text-[15px] font-extrabold leading-[1.3] tracking-tight text-dark-navy sm:text-[16px]">
          <Link href={href} className="line-clamp-2 transition-colors hover:text-primary-yellow">
            {post.title}
          </Link>
        </h3>
