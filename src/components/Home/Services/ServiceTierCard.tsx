...
const linkClassName =
  "text-[17px] font-normal leading-none text-[#2997ff] transition-opacity hover:opacity-80";
...
export function ServiceTierCard({ tier }: ServiceTierCardProps) {
  ...
  return (
    <article
      className={cn(
        "flex min-h-[580px] flex-col overflow-hidden rounded-[22px] sm:min-h-[620px] sm:rounded-[24px]",
    ...
    >
      <div className="flex flex-col items-center px-6 pt-14 text-center sm:px-8 sm:pt-16 md:pt-[3.75rem]">
        ...
        <h3
          className={cn(
            "mt-2 max-w-[18rem] whitespace-pre-line text-[2.5rem] font-semibold leading-[1.08] tracking-[-0.02em] sm:mt-2.5 sm:max-w-none sm:text-[3rem] md:text-[3.25rem]",
        ...
        <div className="mt-5 flex items-center justify-center gap-6 sm:mt-6">
          <Link href={tier.learnMoreHref} className={linkClassName}>
            Learn more &gt;
          </Link>
          <Link href={tier.bookHref} className={linkClassName}>
            Book &gt;
          </Link>
        </div>
      </div>
