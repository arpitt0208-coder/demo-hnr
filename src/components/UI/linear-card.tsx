      ...
      {isOpen && (
        <>
          <motion.div
            ...
            className="fixed inset-0 z-50 h-full w-full bg-white/60 backdrop-blur-sm"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            ...
            <React.Fragment key={item.id}>
              ...
              >
                <DialogTrigger
                  ...
                  className="flex w-full flex-col overflow-hidden border border-[#EEF2F6] bg-white shadow-[0_2px_14px_rgba(15,23,42,0.05)] transition-shadow duration-300 hover:shadow-[0_8px_28px_rgba(15,23,42,0.08)]"
                >
                  <DialogImage
                    src={item.url.src}
                    alt=""
                  ...
                  <div className="flex flex-grow flex-row items-end justify-between p-4">
                    <div>
                      <DialogTitle className="text-left text-lg font-extrabold text-dark-navy sm:text-xl">
                        {item.title}
                      </DialogTitle>
                    </div>
                    <span className="absolute bottom-3 right-3 rounded-full bg-primary-yellow p-2 text-dark-navy">
                      <Plus className="size-5" aria-hidden="true" />
                    </span>
                  </div>
                </DialogTrigger>
                <DialogContainer className="pt-16 sm:pt-20">
                  <DialogContent
                    style={{ borderRadius: '24px' }}
                    className="relative mx-auto flex h-full w-[90%] flex-col overflow-y-auto border border-[#EEF2F6] bg-white shadow-[0_24px_80px_rgba(15,23,42,0.12)] sm:w-[85%] lg:w-[900px]"
                  >
                    <DialogImage
                      src={item.url.src}
                      alt=""
                    ...
                    />
                    <div className="p-6">
                      <DialogTitle className="text-3xl font-extrabold text-dark-navy sm:text-4xl lg:text-5xl">
                        {item.title}
                      </DialogTitle>
                      ...
                      >
                        <p className="mt-3 text-sm font-medium leading-relaxed text-[#475569] sm:text-base">
                          {item.description}
                        </p>
                      </DialogDescription>
                    </div>
                    <DialogClose className="rounded-full bg-dark-navy p-3 text-white hover:bg-dark-navy/90" />
                  </DialogContent>
                </DialogContainer>
              </Dialog>
            </React.Fragment>
